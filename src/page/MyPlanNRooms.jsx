...
function MyPlanNRooms() {
    ...
    const getPlanRooms = async () => {
        try {
            const response = await axios.get(`${process.env.REACT_APP_API_URL}:${process.env.API_PORT}/chat/planNchat`, {
                headers: {
    ...
    const handleCreateRoom = async (planId) => {
        ...
        try {
            ...
            const response = await axios.post(`${process.env.REACT_APP_API_URL}:${process.env.API_PORT}/chat/plan/${planId}`,
                { "name": roomName }, {
                headers: {
                    Authorization: cookies.Authorization
                }, withCredentials: true
